...
nala install autoconf binutils ctags termux-api gh atuin kubectl k9s which neovim nodejs ninja python-tkinter python-greenlet python-cryptography matplotlib python-torch python-pillow python-numpy python-pip ruby golang rust luarocks luajit ripgrep fd lazygit wget gettext logo-ls ncurses-utils libuv stow  libxml2  libxslt timewarrior taskwarrior htop -y || error_exit "${RED}Failed to install packages.${ENDCOLOR}"
